...
def buscarservicio(request):
   # if not request.is_ajax() or request.method != 'POST':
    ...
    model=OrdenPrimaria.objects.filter(servicio=datos['servicio']).filter(fecha_creacion__range=(datos['fechainicio'], datos['fechafin']))
...
def list_json(request, prefijo=None, servicio=None, fechainicio=None, fechafin=None):
    ...
    if prefijo and servicio and fechainicio and fechafin:
        ...
        fecha_inirange.append(fecha_inicio)
        fecha_finrange.append(fecha_fin)
        fecha_inidate=datetime.date(int(fecha_inirange[2]), int(fecha_inirange[0]), int(fecha_inirange[1]))
        fecha_findate=datetime.date(int(fecha_finrange[2]), int(fecha_finrange[0]), int(fecha_finrange[1]))
        #fecha_inidate=datetime.date(int(fechainicio)).strftime('%Y/%m/%d')
        #fecha_findate=datetime.date(int(fechafin)).strftime('%Y/%m/%d')
        model=OrdenHistorico.objects.filter(centro=request.user.trabajador.centro).filter(prefijo=prefijo).filter(servicio=servicio).filter(fecha_defectacion__range=(fecha_inidate, fecha_findate))
        for model in model:
            retorno.append({'No_orden': model.orden_reparada.No_orden, 'servicio': model.prefijo + "" + model.servicio, 'fecha_creacion':model.orden_reparada.fecha_creacion.strftime('%d/%m/%Y'), 'modelo':model.orden_reparada.modelo.nombre, 'serie':model.orden_reparada.serie, 'accion_reparacion': model.accion_reparacion, 'estado': model.estado_defectadas.nombre})
...
def consumir_recursos(request):
    ...
    consumir_recurso=Consumo_Recursos.objects.create(recurso=recurso, centro=request.user.trabajador.centro, fecha=datetime.datetime.now(), cantidad=cantidad, importe=importe, asociado_orden=False)
...
def recursos_consumidos(request):
    retorno = []
    model=Consumo_Recursos.objects.filter(centro=request.user.trabajador.centro).filter(asociado_orden=False)
    for model in model:
        retorno.append({'codigo_sap': model.recurso.codigo_sap, 'descripcion': model.recurso.descripcion, 'fecha':model.fecha.strftime('%Y/%m/%d'), 'precio':model.recurso.precio, 'cantidad':model.cantidad, 'importe': model.importe})
    return JsonResponse({'data_set': retorno})
